...
function parallax.load()
    sprite = love.graphics.newImage("assets/pelle.png")
    x = -255
    y = love.graphics.getHeight() - sprite:getHeight() * scale - 800
    speed = 200
    isPassed = false
    isSpade = false
    isntSpade = false
...
function parallax.update(dt)
    ...
    if x < -300 then -- pour que la pelle revienne a sa position de depart a droite de l'ecran
        x = love.graphics.getWidth() + sprite:getWidth() * scale - 200
        y = love.graphics.getHeight() - sprite:getHeight() * scale - 800
    ...
    elseif x < randomPoint and isPassed == false then -- pour que la pelle se baisse en fonction d'un point random (randomPoint)
        if love.math.random(2) == 1 then -- pour que la pelle se baisse suivant un random de 2
            isSpade = true
        else
            isPassed = true
        end
    ...
    if isSpade == true and isntSpade == false and y < love.graphics.getHeight() - sprite:getHeight() * scale - 480 then -- pour que la pelle se baisse
        y = y + 500 * dt
    end
    if isntSpade == true and y > love.graphics.getHeight() - sprite:getHeight() * scale - 800 then -- pour que la pelle se releve
        y = y - 500 * dt
    end
